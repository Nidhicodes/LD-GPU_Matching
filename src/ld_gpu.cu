    ...
    __device__ size_t warpReduceMax(size_t val, float weight, float *max_weight)
{
    for (int offset = 16; offset > 0; offset /= 2)
    {
        ...
        else if (other_weight == weight && other_val < val)
        {
            // For ties, pick the vertex with the lower ID for deterministic behavior
            // Removed the other_val != SIZE_MAX check as it's unnecessary and can cause issues
            weight = other_weight;
            val = other_val;
        }
    }
...
{
size_t v = edges[e];  // This is a global vertex ID
...
// Set pointer to heaviest available neighbor (global ID)
    ...
    size_t gpu_vertex_offset, int *has_new_matches)
{
    ...
    size_t v = pointers[global_u];  // Get the vertex this one is pointing to (global ID)
    ...
    if (pointers[v] == global_u)
    {
        ...
        if (global_u < v) {
            ...
            if (atomicCAS(reinterpret_cast<unsigned long long*>(&mate[global_u]), 
              static_cast<unsigned long long>(SIZE_MAX), 
              static_cast<unsigned long long>(v)) == static_cast<unsigned long long>(SIZE_MAX))
{
...
__global__ void debugKernel(size_t num_vertices, size_t* pointers, size_t* mate) {
    for (size_t i = 0; i < num_vertices; i++) {
        if (i % 1000 == 0) {  // Print only every 1000th vertex to avoid flooding output
            printf("Vertex %zu points to %zu, matched with %zu\n", 
                   i, pointers[i], mate[i]);
        }
    }
}
...
void LD_GPU_Matcher::debugMatchingState() {
    std::cout << "\n--- DEBUG: Current Matching State ---\n";
    ...
    for (size_t i = 0; i < h_mate.size(); ++i) {
        if (h_mate[i] != SIZE_MAX) {
            if (i < h_mate[i]) {  // Only count each pair once
                matched_count++;
            }
        } else {
            unmatched_count++;
        ...
        // Print some sample vertices and their matches (avoid printing too many)
        if (i < 20 || (i % 1000 == 0 && i < h_mate.size() - 20)) {
            std::cout << "Vertex " << i << " -> ";
            if (h_mate[i] == SIZE_MAX) {
                std::cout << "UNMATCHED";
            } else {
                std::cout << "matched with " << h_mate[i];
            }
            
            if (h_pointers[i] != SIZE_MAX) {
                std::cout << " (points to " << h_pointers[i] << ")";
            }
            std::cout << std::endl;
        }
    }
    ...
    std::cout << "Total unmatched vertices: " << unmatched_count << std::endl;
    std::cout << "-----------------------------------\n\n";
}

void LD_GPU_Matcher::validateMatching() {
    std::cout << "\n--- VALIDATION: Checking Matching Correctness ---\n";
    
    bool valid = true;
    size_t error_count = 0;
    
    // Check if the matching is valid (every vertex is matched to at most one other vertex)
    for (size_t i = 0; i < h_mate.size(); ++i) {
        if (h_mate[i] != SIZE_MAX) {
            // Check if the match is mutual
            if (h_mate[h_mate[i]] != i) {
                std::cout << "ERROR: Vertex " << i << " is matched to " << h_mate[i] 
                          << " but " << h_mate[i] << " is matched to " << h_mate[h_mate[i]] << std::endl;
                valid = false;
                error_count++;
                
                if (error_count >= 10) {
                    std::cout << "Too many errors, stopping validation..." << std::endl;
                    break;
                }
            }
        }
    ...
    if (valid) {
        std::cout << "Matching is valid! All matched pairs are mutual." << std::endl;
    } else {
        std::cout << "Matching is INVALID with " << error_count << " errors detected." << std::endl;
    }
...
bool LD_GPU_Matcher::executeIterationBatched() {
    ...
    for (int gpu = 0; gpu < num_gpus; ++gpu) {
        ...
        for (size_t b = 0; b < gpu_batches.size() - 1; ++b) {
            ...
            for (size_t i = 0; i < batch_size; ++i) {
                vertex_batch[i] = batch_start + i;  // Local vertex ID within partition
            }
    ...
    for (int gpu = 0; gpu < num_gpus; ++gpu) {
        ...
        for (size_t b = 0; b < gpu_batches.size() - 1; ++b) {
            ...
            // Create batch of local vertex IDs within the partition - KEEP CONSISTENT WITH POINTING PHASE
            std::vector<size_t> vertex_batch(batch_size);
            for (size_t i = 0; i < batch_size; ++i) {
                vertex_batch[i] = batch_start + i;  // Local vertex ID within partition
            }
...
void LD_GPU_Matcher::computeMatching() {
    ...
    while (continue_matching) {
        ...
        // Add debugging after each iteration
        if (iteration < 3) {  // Only for first few iterations to avoid too much output
            debugMatchingState();
        }
